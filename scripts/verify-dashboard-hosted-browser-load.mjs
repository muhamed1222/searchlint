...
const sessionStorageNamespace = "searchlint:dashboard-browser-load";
...
if (!address || typeof address === "string") {
  throw new Error("Dashboard hosted browser verifier could not bind server.");
}
...
try {
  ...
  await page.addInitScript(
    ...
    },
    {
      ...
      session: {
        accessToken: "dashboard-browser-load-token",
  ...
  if (heading !== "Project Overview") {
    throw new Error(
      `Dashboard browser verifier expected Project Overview heading, received ${heading}.`
    );
  }
  if (pageErrors.length > 0 || consoleErrors.length > 0) {
    throw new Error(
      [
        "Dashboard hosted browser load produced browser errors.",
        ...pageErrors,
        ...consoleErrors
      ].join("\n")
    );
...
console.log("verified SearchLint dashboard hosted browser load");
  ...
  if (requestUrl.pathname === dashboardPath) {
    const html = renderDashboardHostedHtmlShell({
      ...
      bootstrapConfig: {
        ...
        cognitoClientId: "dashboard-browser-load-client",
        authRoutes: {
          signIn: "/sign-in",
          dashboard: "/dashboard",
    ...
    diagnostics: [
      {
        ...
        evidence: "Fixture diagnostic rendered by browser load verifier.",
        observedAt: "2026-06-22T00:00:00.000Z",
        fingerprint: "dashboard-browser-load-fixture"
      }
    ],
    crawlRuns: [],
    trends: [],
