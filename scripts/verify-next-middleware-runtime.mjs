...
const reportPath = path.join(
  repoRoot,
  "reports/next-middleware-runtime-report.json"
);
...
const headerRoute = "/middleware-header";
const rewriteSource = "/middleware-rewrite-source";
const rewriteTarget = "/middleware-rewrite-target";
const middlewareHeaderName = "x-searchlint-middleware-runtime";
const middlewareHeaderValue = "active";
const expectedHeaderHeading = "Middleware header route";
const expectedRewriteHeading = "Middleware rewrite target";
...
async function main() {
  assert(
    ...
    `Next middleware runtime verification must run under Node.js 24, got ${process.version}`
  );

  const workDir = await mkdtemp(
    path.join(tmpdir(), "searchlint-next-middleware-runtime-")
  );
  const report = {
    schemaVersion: 1,
    ...
    middleware: {
      ...
      rewriteSource,
      rewriteTarget,
      headerName: middlewareHeaderName,
      headerValue: middlewareHeaderValue
    },
    clean: undefined,
    searchlint: undefined,
    comparison: undefined,
    limitations: [
      "Middleware runtime acceptance covers Next.js 16 App Router, the primary OD-003 development/test target.",
      "The verifier covers deterministic response-header and rewrite middleware behavior.",
      "Middleware auth, cookies, geo, and deployed Edge-provider scenarios are not covered."
    ]
  };
  ...
  try {
    ...
    assert(
      ...
      "middleware header route must return HTTP 200"
    );
    assert(
      clean.header.middlewareHeader === middlewareHeaderValue,
      "clean middleware header must be present"
    );
    assert(
      searchlint.header.middlewareHeader === clean.header.middlewareHeader,
      "SearchLint middleware header must match clean"
    );
    assert(
      clean.header.heading === expectedHeaderHeading,
      "middleware header route must render expected page"
    );
    assert(
    ...
    assert(
      ...
      "middleware rewrite route must return HTTP 200"
    );
    assert(
      clean.rewrite.heading === expectedRewriteHeading,
      "middleware rewrite source must render target page"
    );
    assert(
    ...
    assert(
      ...
      "SearchLint middleware header response must not contain runtime markers"
    );
    assert(
      searchlint.rewrite.containsSearchLintRuntime === false,
      "SearchLint middleware rewrite response must not contain runtime markers"
    );
  ...
  console.log(`Next middleware runtime ${report.status}`);
...
async function writeFixtureProject(projectDir, packages, searchlint) {
  await mkdir(path.join(projectDir, "app", "middleware-header"), {
    recursive: true
  });
  await mkdir(path.join(projectDir, "app", "middleware-rewrite-target"), {
    recursive: true
  });
...
`
  ...
  await writeFile(
    path.join(projectDir, "middleware.js"),
...
export function middleware(request) {
  ...
  response.headers.set("${middlewareHeaderName}", "${middlewareHeaderValue}");
  return response;
}
...
`
  ...
  await writeFile(
    path.join(projectDir, "app", "middleware-header", "page.jsx"),
    `export default function MiddlewareHeaderPage() {
  return <main><h1>${expectedHeaderHeading}</h1></main>;
}
`
  );
  await writeFile(
    path.join(projectDir, "app", "middleware-rewrite-target", "page.jsx"),
    `export default function MiddlewareRewriteTargetPage() {
...
async function fetchHtml(url) {
  const response = await fetch(url, {
    headers: {
      "user-agent": "Mozilla/5.0 SearchLintMiddlewareRuntimeAcceptance"
    }
  });
  ...
  return {
    ...
    middlewareHeader: response.headers.get(middlewareHeaderName) ?? "",
