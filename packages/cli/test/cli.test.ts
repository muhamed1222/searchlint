...
describe("runSearchLintCli", () => {
  ...
  it("prints version diagnostics", async () => {
    ...
    expect(result).toMatchObject({
      exitCode: 0,
      stdout: "searchlint 1.0.0-beta.22\n",
      stderr: ""
    });
  });
  ...
  it("prints doctor diagnostics", async () => {
    ...
    expect(result.stderr).toBe("");
    expect(result.stdout).toContain("SearchLint doctor");
    expect(result.stdout).toContain("version: 1.0.0-beta.22");
  ...
  it("infers the starter site URL from package homepage", async () => {
    const files: Record<string, string> = {
      "package.json": JSON.stringify({
  ...
  it("prefers init --site over package homepage", async () => {
    const files: Record<string, string> = {
      "package.json": JSON.stringify({
        homepage: "https://homepage.example",
        scripts: { dev: "next dev" },
  ...
  it("prints yarn next steps for yarn lockfile projects", async () => {
    const files: Record<string, string> = {
      "package.json": JSON.stringify({
        scripts: { dev: "next dev" },
        dependencies: { next: "16.2.9" }
    ...
    expect(result.stdout).toContain("  yarn searchlint:verify");
